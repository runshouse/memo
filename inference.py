...
def main():
    ...
    # Measure time taken to load the config
    ...
    logger.info(f"Model paths determined in {time.time() - start_time:.2f} seconds")
    ...
    logger.info(f"Device set up in {time.time() - start_time:.2f} seconds")
    ...
    logger.info(f"Audio processed in {time.time() - start_time:.2f} seconds")

    # Processing audio emotion
    start_time = time.time()
    ...
    logger.info(f"Audio emotion processed in {time.time() - start_time:.2f} seconds")
    ...
    logger.info(f"xFormers memory-efficient attention enabled in {time.time() - start_time:.2f} seconds")
    ...
    logger.info(f"Inference pipeline created in {time.time() - start_time:.2f} seconds")
    ...
    logger.info(f"Final video processing completed in {time.time() - start_time:.2f} seconds")
